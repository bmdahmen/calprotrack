...
export default {
  async fetch(request, env) {
    ...
    if (path === '/auth/profile') {
      ...
      const { tdee, pro_target, age, weight_lbs, height_in, activity, goal_mode, aggressiveness, theme, dexa_date, dexa_weight, dexa_bf_pct, muscle_loss_pct, muscle_gain_pct, muscle_maintain_pct, rmr, cal_target_override, coach_mode, diet_notes, track_macros, fat_target_override, carbs_target_override } = body;
      await env.DB.prepare(
        'UPDATE users SET tdee=?, pro_target=?, age=?, weight_lbs=?, height_in=?, activity=?, goal_mode=?, aggressiveness=?, theme=?, dexa_date=?, dexa_weight=?, dexa_bf_pct=?, muscle_loss_pct=?, muscle_gain_pct=?, muscle_maintain_pct=?, rmr=?, cal_target_override=?, coach_mode=?, diet_notes=?, track_macros=?, fat_target_override=?, carbs_target_override=? WHERE id=?'
    ...
    if (path === '/fooditems/save') {
      ...
      for (let i = 0; i < food_items.length; i++) {
        ...
        stmts.push(env.DB.prepare('INSERT INTO food_items (id,user_id,name,cal,pro,weight_g,fat,carbs,hidden,sort_order,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
          .bind(String(f.id), user_id, f.name, f.cal, f.pro || 0, f.weight_g || null, f.fat ?? null, f.carbs ?? null, f.hidden ? 1 : 0, i, f.created_at || new Date().toISOString()));
      }
      await env.DB.batch(stmts);
      return json({ ok: true });
    }
